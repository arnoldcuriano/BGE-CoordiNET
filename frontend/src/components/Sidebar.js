...
const Sidebar = ({
  ...
  const backgroundGradient = isDarkMode
    ...
    : "linear-gradient(190deg, #e3ffe7 0%, #d9e7ff 100%)";
  ...
  const user = useMemo(
    () =>
      propUser || {
        role: authState.role || authState.userRole,
        ...
        <>
          ...
          <List sx={{ flexGrow: 1, overflow: "auto" }}>
            {menuItems.map((item) => {
              ...
              const hasPermission =
                isSuperAdmin || accessPermissions[item.permissionKey] === true;
