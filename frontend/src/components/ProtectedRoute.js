...
const ProtectedRoute = ({ children, isPublic = false, pageKey }) => {
  ...
  useEffect(() => {
    ...
    if (!authState.isAuthenticated && !isPublic && !authState.loading) {
      console.log('ProtectedRoute: Triggering fetchUser due to unauthenticated state');
      fetchUser();
    }
  ...
  if (authState.loading) {
    console.log('ProtectedRoute: authState is still loading, showing loading indicator');
    return (
      <Box
        sx={{
          display: 'flex',
  ...
  if (authState.userRole === 'superadmin') {
    ...
    if (location.pathname === '/no-access' || location.pathname === '/welcome') {
      const intendedRoute = location.state?.from?.pathname || '/superadmin-dashboard';
      console.log('ProtectedRoute: Superadmin on /no-access or /welcome, redirecting to:', intendedRoute);
  ...
  if (isPublic) {
    ...
    if (authState.isAuthenticated) {
      ...
      if (!authState.isApproved) {
        ...
        return <Navigate to="/welcome" state={{ message: 'Your account is awaiting approval.', from: location }} replace />;
      }
      ...
      if (hasCoreAccess) {
        ...
        const redirectState = accessibleRoute ? { from: location } : { message: 'Welcome! Please wait for the Super Admin to assign your permissions.', from: location };
        ...
        return <Navigate to="/welcome" state={{ message: 'Welcome! Please wait for the Super Admin to assign your permissions.', from: location }} replace />;
      }
    }
  ...
  if (pageKey) {
    ...
    console.log('ProtectedRoute: Checking access for pageKey:', pageKey, 'hasAccess:', hasAccess);
    if (!hasAccess) {
      console.log('ProtectedRoute: Access denied to', pageKey, 'redirecting to /welcome');
      return <Navigate to="/welcome" state={{ message: 'You do not have access to this page. Please contact an administrator.', from: location }} replace />;
    }
  }

  console.log('ProtectedRoute: User has permission, allowing access to', location.pathname);
  return children;
};
