...
import { Typography, Box, Divider, List, ListItem, ListItemText, CircularProgress, Grid } from '@mui/material';
...
const PatchNotes = () => {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const response = await axios.get('/api/patch-notes');
        if (response.data.length !== patchNotes.length || response.data.some((note, index) => note._id !== patchNotes[index]?._id)) {
          setPatchNotes(response.data);
          if (response.data && response.data.length > 0) {
            setSelectedPatch(response.data[0]);
          }
                  ...
                  {patchNotes.map((patch, index) => (
                    ...
                    >
                      <ListItemText
                        ...
                        primary={patch.title}
                        secondary={patch.publishedAt ? new Date(patch.publishedAt).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'short',
                          day: 'numeric',
                        }) : 'Unpublished'}
              ...
              >
                {selectedPatch ? (
                  <>
                    ...
                    >
                      Published on {new Date(selectedPatch.publishedAt).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                      })}
