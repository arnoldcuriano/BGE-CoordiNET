...
import ManagePatchNotes from './ManagePatchNotes';
...
const SuperAdminDashboard = () => {
  console.log('SuperAdminDashboard: Setting up routes');
  ...
  // Role check moved outside of useEffect to prevent early redirection
  // Add a loading state for authState to handle async initialization
  const [authChecked, setAuthChecked] = useState(false);

  useEffect(() => {
    console.log('SuperAdminDashboard: authState:', authState);
    // Wait until authState is fully initialized (loading: false)
    if (authState.loading) {
      console.log('SuperAdminDashboard: authState is still loading');
      return;
    }

    if (!authState.isAuthenticated || authState.userRole !== "superadmin") {
      console.log('SuperAdminDashboard: Redirecting to /dashboard due to role check failure');
      navigate("/dashboard");
      return;
    }

    setAuthChecked(true); // Auth check passed, proceed to render
  ...
  const filteredApprovedMembers = useMemo(() => {
    ...
    return sortData(
      ...
      approvedSortOrder,
      (member) => {
        if (approvedSortField === "name")
          return `${member.firstName || ""} ${
            member.lastName || ""
          }`.toLowerCase();
        if (approvedSortField === "email")
          return (member.email || "").toLowerCase();
        if (approvedSortField === "role")
  ...
  const handleApproveMember = async (userId, role) => {
    try {
      ...
      const fetchApproved = await axios.get("/auth/members", {
        withCredentials: true,
      });
      const fetchPending = await axios.get("/auth/pending-users", {
        withCredentials: true,
      });
      setApprovedMembers(
        fetchApproved.data.filter((m) => m && typeof m === "object" && m._id)
      );
      setPendingMembers(
        fetchPending.data.filter((m) => m && typeof m === "object" && m._id)
      );
    } finally {
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      const membersToApprove = pendingMembers.filter((m) =>
        selectedPending.includes(m._id)
      );
      const defaultRole = "viewer";
      setPendingMembers((prev) =>
        prev.filter((m) => !selectedPending.includes(m._id))
      );
      setApprovedMembers((prev) => [
        ...prev,
        ...
        selectedPending.map((userId) =>
          axios.post(
            "/auth/approve-user",
            {
              ...
              accessPermissions:
                roleDefaults[selectedRoles[userId] || defaultRole],
            },
            { withCredentials: true }
          )
        )
  ...
  const handleBulkReject = async () => {
    ...
    try {
      setBulkLoading(true);
      setPendingMembers((prev) =>
        prev.filter((member) => !selectedPending.includes(member._id))
      );
      setSelectedPending([]);
      await Promise.all(
        selectedPending.map((userId) =>
      ...
      setSnackbar({
        open: true,
        message: `${selectedPending.length} members rejected`,
        severity: "success",
      });
    } catch (error) {
      console.error(
        "Error bulk rejecting members:",
        error.response?.data || error.message
      );
      setSnackbar({
        open: true,
        message: "Failed to reject members",
        severity: "error",
      });
      const fetchPending = await axios.get("/auth/pending-users", {
        withCredentials: true,
  ...
  const handleSaveAccess = async () => {
    ...
    if (selectedMember.role === "superadmin") {
      setSnackbar({
        open: true,
        message: "Cannot change access for superadmin members",
        severity: "warning",
      });
      return;
    }
      ...
      setApprovedMembers((prev) =>
        prev.map((m) =>
          m._id === selectedMember._id
            ? { ...m, accessPermissions: tempPermissions }
            : m
        )
      );
      setSnackbar({
  ...
  return (
    <Layout user={user}>
      <Box sx={{ display: 'flex', minHeight: '100vh' }}>
        {/* Sidebar is already included via Layout */}
        <Box sx={{ flexGrow: 1 }}>
          <Routes>
            {/* Default route for /superadmin-dashboard */}
            <Route
              path="/"
              element={
                <Box
                      ...
                      <>
                        ...
                        <Box sx={{ position: "relative" }}>
                          ...
                          >
                            <Table
                              sx={{ minWidth: 650 }}
                              aria-label="pending members table"
                            >
                              <TableHead>
                                <TableRow>
                                  <TableCell
                                      ...
                                      >
                                        ...
                                        <Tooltip title="Approve this user">
                                          <Button
                                            ...
                                            disabled={
                                              actionLoading[member._id] || bulkLoading
                                            }
                                            sx={{
                                              mr: 1,
                                              fontFamily: muiTheme.typography.fontFamily,
                                              ...
                                              "&:hover": {
                                                ...
                                                boxShadow:
                                                  muiTheme.custom.shadows.buttonHover,
                                              },
                                              "&:disabled": {
                                                background:
                                                  "linear-gradient(90deg, #4285F4, #34A853)",
                              ...
                              >
                                <FormControlLabel
                                  control={
                                    <Checkbox
                                      onChange={(e) =>
                                        handleBulkAccessChange(page.key, e.target.checked)
                                      }
                                      disabled={
                                        selectedApproved.length === 0 ||
                                        hasSuperadminSelected ||
                                        bulkLoading
                                      }
                                      sx={{
                                        color: muiTheme.palette.text.primary,
                                    ...
                                    >
                                      ...
                                      >
                                        <Checkbox
                                          ...
                                          onChange={() =>
                                            handleSelectApproved(member._id)
                                          }
                                          sx={{
                                            color: muiTheme.palette.text.primary,
                                            "&.Mui-checked": { color: greenLightColor },
                    ...
                    >
                      <Fade in={openModal}>
                        ...
                        >
                          ...
                          >
                            Edit Access for {selectedMember?.firstName}{" "}
                              ...
                              <FormControlLabel
                                ...
                                control={
                                  <Checkbox
                                    ...
                                    onChange={(e) =>
                                      handleAccessChange(page.key, e.target.checked)
                                    }
                                    sx={{
                                      color: muiTheme.palette.text.primary,
                                      "&.Mui-checked": { color: greenLightColor },
                    ...
                    >
                      <Fade in={deleteConfirmModal.open}>
                        ...
                        >
                          ...
                          >
                            Are you sure you want to delete {deleteConfirmModal.userName}?
                          ...
                          >
                            ...
                            <Button
                              onClick={() =>
                                handleDeleteMember(deleteConfirmModal.userId)
                              }
                              variant="contained"
                              color="error"
                              sx={{
                      ...
                      <Fade in={approveConfirmModal.open}>
                        ...
                        >
                          ...
                          >
                            ...
                            {approveConfirmModal.userName} as a {approveConfirmModal.role}
                            ?
                          </Typography>
                          <Box
                            sx={{ display: "flex", gap: 2, justifyContent: "flex-end" }}
                    ...
                    >
                      <Fade in={rejectConfirmModal.open}>
                        ...
                        >
                          ...
                          >
                            Are you sure you want to reject {rejectConfirmModal.userName}?
                            This action cannot be undone.
                          </Typography>
                          <Box
                            sx={{ display: "flex", gap: 2, justifyContent: "flex-end" }}
                          >
                            ...
                            <Button
                              onClick={() =>
                                handleRejectMember(rejectConfirmModal.userId)
                              }
                              variant="contained"
                              color="error"
                              sx={{
            ...
            />
            {/* Route for /superadmin-dashboard/patch-notes */}
           <Route path="patch-notes" element={<ErrorBoundary><ManagePatchNotes /></ErrorBoundary>} />
          </Routes>
        </Box>
      </Box>
    </Layout>
