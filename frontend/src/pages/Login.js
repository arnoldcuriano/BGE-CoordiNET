...
const Login = () => {
  ...
  useEffect(() => {
    if (authState.isAuthenticated) {
      ...
      if (!authState.isApproved) {
        ...
        navigate("/welcome", {
          state: { message: "Your account is awaiting approval." },
          replace: true,
        });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage("");
    try {
      console.log("Login.js: Calling login function:", {
        email,
        password,
        rememberMe,
      });
      ...
      if (
        ...
        error.response?.data?.message === "Your account is awaiting approval."
      ) {
        navigate("/welcome", {
  ...
  if (loading || googleLoading) {
    console.log(
      "Login.js: Form submission in progress, showing loading indicator"
    );
    return (
      <Box
        sx={{
          ...
          <Box sx={{ mt: "30px" }}>
            {[
              ...
              >
                {item.icon}
                <Box
                  sx={{ flexGrow: 1, marginLeft: { xs: "12px", sm: "16px" } }}
                >
                  <Typography
                    variant="h5"
                    sx={{
