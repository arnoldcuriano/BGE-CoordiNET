...
const computePartnerStatus = (partner) => {
  ...
  const hasTermSheet = partner.services.some(
    (service) => service.termSheets && service.termSheets.length > 0
  );
  if (hasTermSheet) {
    return "Hot";
  }
...
router.get("/notifications", isAuthenticated, async (req, res) => {
  try {
    const notifications = await PartnerNotification.find().sort({
      createdAt: -1,
    });
    if (!notifications) {
...
router.get("/:id/activity-logs", isAuthenticated, async (req, res) => {
  try {
    const logs = await ActivityLog.find({ partnerId: req.params.id }).sort({
      timestamp: -1,
    });
    if (!logs || logs.length === 0) {
      return res
        .status(404)
        .json({ message: "No activity logs found for this partner" });
    }
    res.json(logs);
  } catch (err) {
      ...
      for (const service of partner.services) {
        if (service.termSheets && service.termSheets.length > 0) {
          for (const termSheet of service.termSheets) {
            try {
              const fileKey = termSheet.split(
                "bge-user-files.s3.amazonaws.com/"
              )[1];
              const deleteParams = {
                Bucket: bucket,
                Key: fileKey,
              ...
              console.error(
                "Error deleting term sheet during partner deletion:",
                deleteError
              );
            }
          }
...
router.put("/:id/status", isAuthenticated, async (req, res) => {
  try {
    ...
    console.log("Updated status via /status endpoint:", {
      before: beforeStatus,
      after: status,
    });
      ...
      bufferStream
        ...
        .on("data", (record) => {
          records.push({
            ...
            status: ["Hot", "Warm", "Cold"].includes(record.status)
              ? record.status
              : "Cold",
            ...
            durationStatus: [
              ...
              "Expired",
              "Custom",
            ].includes(record.durationStatus)
              ? record.durationStatus
              : "Upcoming",
        ...
        .on("end", () => {
          console.log("Parsed CSV for preview:", {
        ...
        .on("end", async () => {
          ...
          for (const [index, record] of records.entries()) {
            try {
              ...
              if (
                !partnerData.name ||
                !partnerData.projectName ||
                !partnerData.startDate ||
                !partnerData.endDate
              ) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (req.files && req.files.length > 0) {
        ...
        for (const file of req.files) {
          if (!allowedTypes.includes(file.mimetype)) {
            return res
              .status(400)
              .json({ message: "Only PDF, DOCX, and XLSX files are allowed." });
          }
          if (file.size > maxSize) {
            return res
              .status(400)
              .json({ message: "File size exceeds 10 MB limit." });
          }

          const fileExtension = file.originalname.split(".").pop();
          const fileName = `partner-term-sheet/${Date.now()}-${
            req.params.id
      ...
      const addedService =
        updatedPartner.services[updatedPartner.services.length - 1];
      ...
      await logActivity(
        ...
        partner.projectName,
        {
          ...
          "service.description": {
            before: null,
            after: newService.description || "N/A",
          },
  ...
  async (req, res) => {
    try {
      ...
      if (before.description !== service.description) {
        changes["service.description"] = {
          before: before.description || "N/A",
          after: service.description || "N/A",
        };
      }
      if (before.cost !== service.cost) {
        changes["service.cost"] = {
          before: before.cost || "N/A",
          after: service.cost || "N/A",
        };
      }
      if (before.status !== service.status) {
        changes["service.status"] = {
          before: before.status || "N/A",
          after: service.status || "N/A",
        };
      }
      if (
        JSON.stringify(before.termSheets) !== JSON.stringify(service.termSheets)
      ) {
        changes["service.termSheets"] = {
          before: before.termSheets.length,
          after: service.termSheets.length,
        };
      }
...
router.delete(
  "/:id/services/:serviceId/term-sheet",
  isAuthenticated,
  async (req, res) => {
    try {
      const user = await User.findById(req.user._id);
      ...
      if (!termSheetUrl || !service.termSheets.includes(termSheetUrl)) {
        return res
          .status(404)
          .json({ message: "Term Sheet not found in service" });
      }

      try {
        const fileKey = termSheetUrl.split(
          "bge-user-files.s3.amazonaws.com/"
        )[1];
        const deleteParams = {
          Bucket: bucket,
      ...
      service.termSheets = service.termSheets.filter(
        (url) => url !== termSheetUrl
      );
    ...
    if (service.termSheets && service.termSheets.length > 0) {
      for (const termSheet of service.termSheets) {
        try {
          ...
          console.error(
            "Error deleting term sheet during service deletion:",
            deleteError
          );
        }
      }
...
router.post("/:id/milestones", isAuthenticated, async (req, res) => {
  try {
    ...
    const addedMilestone =
      updatedPartner.milestones[updatedPartner.milestones.length - 1];
    ...
    await logActivity(
      ...
      partner.projectName,
      {
        ...
        "milestone.description": {
          before: null,
          after: newMilestone.description || "N/A",
        },
    ...
    try {
      ...
      if (before.description !== milestone.description) {
        changes["milestone.description"] = {
          before: before.description || "N/A",
          after: milestone.description || "N/A",
        };
      }
      if (before.date !== milestone.date) {
        changes["milestone.date"] = {
          before: before.date,
          after: milestone.date,
        };
      }
...
router.put("/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    partner.status =
      req.body.overrideStatus && req.body.status
        ? req.body.status
        : req.body.status || partner.status;
    partner.manualStatus =
      req.body.overrideStatus && req.body.status ? true : partner.manualStatus;
    ...
    partner.durationStatusCustom =
      req.body.durationStatusCustom || partner.durationStatusCustom;
    ...
    ["email", "phone", "person", "role"].forEach((field) => {
      if (before.contact[field] !== partner.contact[field]) {
        changes[`contact.${field}`] = {
          before: before.contact[field] || "N/A",
          after: partner.contact[field] || "N/A",
        };
      }
    });
